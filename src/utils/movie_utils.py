...
__all_episode_patterns = [
    re.compile("全(%s)[集期]" % __num_pattern),
    re.compile("(%s)[集期]全" % __num_pattern),
...
__all_season_patterns = [re.compile("全(%s)[季部辑]" % __num_pattern)]
...
def series_title_match(searched_clean_title, douban_title_list):
    for title in douban_title_list:
        [lenMatch, strMatch] = get_Max_common_substr(
            searched_clean_title, get_clean_title(title)
        )
...
def parse_episode_by_title(title, total_episode):
    ...
    return {
        "season": {
            "start": season_start,
            "end": season_end,
            "complete": match_all_season,
            "index": season_index,
        },
        "ep": {
            "start": ep_start,
            "end": ep_end,
            "complete": match_all_ep,
            "auto_ep": auto_ep,
            "index": ep_index,
            "total_count": total_episode,
        },
    }
...
if __name__ == "__main__":
    test = [
        "权力的游戏 2019 第八季【4K UHD 港版原盘原生中字】",
    ...
    for s in ["小猪佩奇第七季.Peppa.Pig.Season.7.Complete.4K.WEB-DL.H265.AAC-OurTV"]:
        ...
        print(
            ...
            % (
                data["season"]["start"],
                data["season"]["end"],
                "全季" if data["season"]["complete"] else "未完结",
                data["ep"]["start"],
                data["ep"]["end"],
                "全集" if data["ep"]["complete"] else "未播完",
                s,
            )
        )
