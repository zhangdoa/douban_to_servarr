...
class DoubanMovieCrawler:
    ...
    def get_movie_details(self, url):
        ...
        if len(found_imdb_ids) > 0:
            ...
            logger.error('Can\'t find the IMDB ID for {}', url)
            return None
        ...
        genres = []
        for cate in found_genres:
            genres.append(cate)
        
        found_release_dates = re.findall(r'<span property="v:initialReleaseDate" content="(\d+-\d+-\d+)\(([^)]+)\)">', text)
        release_dates = []
        ...
        for type in types:
            ...
            while uri is not None:
                ...
                for i in range(len(movie_list_a)):
                    ...
                    days = (datetime.datetime.now() - datetime.datetime.strptime(added_date, "%Y-%m-%d")).days
                    ...
                    if len(titles) > 1:
                        ...
                        original_title = None
                    found_imdb_ids = re.search(r'/subject/(\d+)', url)
                    if found_imdb_ids:
                        imdb_id = found_imdb_ids.group(1)
                    else:
                        imdb_id = None
                    result.append(
            ...
            logger.info('{}天之内加入{}的影视，共{}部', within_days, type, len(result))
        return all_result
